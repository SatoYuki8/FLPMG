...
int wrapper_adder(int width, FILE *fp, char *module_name){
  fprintf(fp,
	  "declare %s{\n"
	  "input a<%d>, b<%d>;\n"
	  "output result<%d>;\n"
...
P_IF
  fprintf(fp,
	  ...
	  "bshift.do_in((ymsb || Ab<%d:0> || 0b000), exp_diff).f,\n"
	  "Aa<%d>,\n"
	  "Ab<%d>,\n"
	  "Aa<%d:%d>\n"
	  ");\n"
...
P_ELSE
  fprintf(fp,
	  ...
	  "Bm2 = bshift.do_in((ymsb || Ab<%d:0> || 0b000), exp_diff).f;\n"
	  "Bs1 = Aa<%d>;\n"
	  "Bs2 = Ab<%d>;\n"
	  "Bexp = Aa<%d:%d>;\n"
	  "\n",
...
P_IF
  fprintf(fp,
	  "stage B{\n"
	  "par{\n"
	  "m3 = madd.do_in(Bs1, Bs2, Bm1, Bm2).r1;\n"
	  "alt{\n"
	  "(m3<%d>): par{\n"
...
P_IF
  fprintf(fp,
	  ...
	  "alt{\n"
	  "(Dm<%d>): par{\n"
	  "rounded_frac = incfrac.do_in(Dm<2>&(Dm<3>|Dm<1>|Dm<0>), Dm<%d:3>).out;\n"
	  "result = ( Ds || (Dexp + incfrac.p) || rounded_frac );\n"
	  "finish;\n"
	  "}\n"
	  "else: par {\n"
	  "result = (%d#0b0);\n"
...
int FPAdder_Generator(int exp, int frac, int width, FILE *fp, flags_t flag, char *module_name){
...
int Manage_FPAdder_Generator(int argc, char **argv){  
  ...
  if((flag->step1_flag == 1) |     /* When it measure step latency */
     (flag->step2_flag == 1) |
     (flag->step3_flag == 1) |
     (flag->step4_flag == 1) |
     (flag->step5_flag == 1) 
     ){
    fpadder_step_measurement(exp, frac, width, fp, flag, top_module_name);
    flag->wrapper_flag = 0;   //wrapperモジュールが2つできてしまうのを防ぐため
  }else if (flag->fpadd_flag == 1){           /* When it generates FPadder module  */
    FPAdder_Generator(exp, frac, width, fp, flag, top_module_name);
  ...
  if (flag->wrapper_flag == 1){
    wrapper_adder(width, fp, top_module_name);
